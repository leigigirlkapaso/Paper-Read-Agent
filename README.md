...
其他可调参数（保持默认即可跑通第一次）：
- `sources.openalex: true` — OpenAlex 免费，2.5 亿+ 文献，建议保留开启
- `sources.dblp: false` — DBLP 覆盖 CS 顶会，HCI 等非 arXiv 学科建议开启
- `max_search_results: 200` — 每条 query 返回的候选文献数
- `relevance_threshold: 0.8` — 相关性筛选阈值（0-1）
...
AGENT1: 关键词提取（LLM 三层策略）
    │
    ├── arxiv API 检索（主力，免费）
    ├── OpenAlex API（2.5 亿+ 文献兜底，免费）
    └── DBLP API（CS 顶会/期刊，HCI 等非 arXiv 学科）
    └── Europe PMC API（生物医学，4200 万+，免费）
    │
    ▼
AGENT1-B: LLM 批量打分筛选（相关性 0-1 分）
    │
    ▼
多源 PDF 下载（arXiv → Unpaywall → Sci-Hub）
    │
    ▼
...
- `llm` — 大语言模型 API
- `research` — 研究构想 + 检索参数
- `sources` — 四平台开关（arxiv / openalex / dblp / pmc）
...
这是正常的。arxiv API 要求请求间隔 ≥3 秒，三平台并行检索后总耗时约等于最慢的平台。可以暂时关掉不需要的平台：
