...
from agent1.dblp_searcher import search_dblp, search_dblp_with_conferences
...
def run_online_search(
    ...
    if dblp_enabled:
        dblp_conferences = sources_cfg.get("dblp_conferences", None)
        # 兼容旧格式：纯字符串列表 → 转为 dict 列表
        if dblp_conferences and isinstance(dblp_conferences[0], str):
            dblp_conferences = [{"name": c, "aliases": []} for c in dblp_conferences]

        # 校验 dblp_conferences 格式，防止 KeyError: 'name'
        if dblp_conferences:
            validated = []
            for i, conf in enumerate(dblp_conferences):
                if not isinstance(conf, dict) or "name" not in conf:
                    logger.error(
                        f"dblp_conferences[{i}] 格式错误，期望 {{name: str, aliases: [str]}}，实际: {conf}"
                    )
                    db.log(session_id, "ERROR",
                        f"dblp_conferences 配置格式错误：第 {i+1} 项缺少 'name' 字段。"
                        f"正确格式: {{name: \"会议名\", aliases: [\"别名1\"]}}，"
                        f"当前值: {json.dumps(conf, ensure_ascii=False) if isinstance(conf, dict) else repr(conf)}")
                    continue
                validated.append(conf)
            dblp_conferences = validated if validated else None

        def _run_dblp():
            if dblp_conferences:
                return search_dblp_with_conferences(
                    queries=queries,
                    conferences=dblp_conferences,
                    max_results_per_query=sources_cfg.get("dblp_max_per_query", 100),
                    min_year=min_year,
                    query_delay=sources_cfg.get("dblp_query_delay", 2.0),
                    max_queries=research_cfg.get("max_queries", 8),
                    search_years=sources_cfg.get("dblp_search_years", 3),
                    search_all=sources_cfg.get("dblp_search_all", False),
                )
            else:
                return search_dblp(
    ...
    download_results = download_papers_batch_multi(
        ...
        enable_institutional=downloader_cfg.get("enable_institutional", False),
