...
def _toggle_thinker(core, enable: bool) -> bool:
    """持久化 thinker 全局组件开关状态。返回新状态。"""
    content = "enabled" if enable else "disabled"
    try:
        core.db.conn.execute(
            "DELETE FROM core_notes WHERE source_module = 'thinker' AND source_ref = 'visibility'"
        )
        core.db.conn.execute(
            "INSERT INTO core_notes (source_module, source_ref, content, content_type) "
            "VALUES ('thinker', 'visibility', ?, 'config')",
            (content,),
        )
        core.db.conn.commit()
    except Exception:
        logger.warning("[Thinker] 无法持久化 visibility 状态")

    core._thinker_visible = enable
    if enable:
        ...
        core.frontend.unregister_module("thinker")
    return enable


@router.get("/api/enabled")
async def get_enabled(request: Request):
    core = request.app.state.core
    visible = getattr(core, "_thinker_visible", True)
    return JSONResponse({"enabled": visible})


@router.post("/api/toggle")
async def toggle_thinker(request: Request):
    core = request.app.state.core
    current = getattr(core, "_thinker_visible", True)
    new_state = _toggle_thinker(core, not current)

    if request.headers.get("HX-Request"):
        target = request.headers.get("HX-Target", "")
        if "thinker-toggle-btn" in target:
            # Settings page pill button + status text + instant hide/show
            color = "bg-indigo-600" if new_state else "bg-gray-300"
            pos = "translate-x-6" if new_state else "translate-x-1"
            status = "已开启" if new_state else "已关闭"
            show_hide_js = (
                '<script>document.querySelectorAll("[data-module=thinker]").forEach('
                'function(el){el.style.display="";});</script>'
                if new_state else
                '<script>document.querySelectorAll("[data-module=thinker]").forEach('
                'function(el){el.style.display="none";});</script>'
            )
            return HTMLResponse(
                f'<button id="thinker-toggle-btn" class="relative inline-flex h-6 w-11 '
                f'items-center rounded-full transition-colors {color}" '
                f'hx-post="/thinker/api/toggle" hx-swap="outerHTML" hx-target="#thinker-toggle-btn">'
                f'<span class="inline-block h-4 w-4 transform rounded-full bg-white '
                f'transition-transform {pos}"></span></button>'
                f'<span id="thinker-status-text" hx-swap-oob="true">{status}</span>'
                f'{show_hide_js}'
            )
        # Nav bar icon button (default) + instant hide/show
        color = "text-indigo-500 hover:text-indigo-400" if new_state else "text-gray-300 hover:text-gray-400"
        icon = "🤖" if new_state else "💤"
        title = "关闭小思" if new_state else "开启小思"
        show_hide_js = (
            '<script>document.querySelectorAll("[data-module=thinker]").forEach('
            'function(el){el.style.display="";});</script>'
            if new_state else
        ...
        return HTMLResponse(
            f'<button id="thinker-nav-toggle" class="text-lg transition-colors {color}" '
            f'hx-post="/thinker/api/toggle" hx-swap="outerHTML" hx-target="#thinker-nav-toggle" '
