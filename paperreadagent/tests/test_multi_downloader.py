...
class TestCascadeOrder:
    def test_arxiv_only(self):
        cascade = _build_cascade("2301.07041", "", "", "", False, False, None)
        sources = [s for s, _ in cascade]
        assert sources == ["arxiv"]

    def test_arxiv_with_direct_url(self):
        cascade = _build_cascade("2301.07041", "https://example.com/paper.pdf", "", "", False, False, None)
        sources = [s for s, _ in cascade]
        assert sources == ["arxiv", "direct"]

    def test_no_arxiv_no_doi(self):
        cascade = _build_cascade("oa_W123456", "", "", "", False, False, None)
        assert cascade == []

    def test_doi_unlock_unpaywall_and_s2(self):
        cascade = _build_cascade("s2_abc", "", "10.1234/test", "user@test.com", False, False, None)
        sources = [s for s, _ in cascade]
        assert sources == ["unpaywall", "s2_oa"]

    def test_institutional_enabled(self):
        cascade = _build_cascade("s2_abc", "", "10.1234/test", "", False, True, None)
        sources = [s for s, _ in cascade]
        assert sources == ["publisher", "s2_oa"]

    def test_scihub_enabled(self):
        cascade = _build_cascade("oa_W123", "", "10.1234/test", "", True, False, None)
        sources = [s for s, _ in cascade]
    ...
    def test_full_cascade(self):
        cascade = _build_cascade(
            ...
            "10.1234/test", "user@test.com", True, True, ["https://sci-hub.se"]
        )
        sources = [s for s, _ in cascade]
        assert sources == ["arxiv", "direct", "publisher", "unpaywall", "s2_oa", "scihub"]
