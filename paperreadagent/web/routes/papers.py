...
@router.get("/fav", response_class=HTMLResponse)
...
@router.post("/{paper_id}/fav")
async def toggle_favorite(request: Request, paper_id: int):
    """切换收藏状态，返回 HTMX 片段（星星按钮）。"""
    db = request.app.state.db
    is_fav = db.toggle_favorite(paper_id)
    star = "★" if is_fav else "☆"
    cls = "text-yellow-500" if is_fav else "text-gray-300 hover:text-yellow-400"
    return HTMLResponse(
        f'<button class="text-xl {cls} transition-colors" '
        f'hx-post="/papers/{paper_id}/fav" '
        f'hx-swap="outerHTML" '
        f'title="{"取消收藏" if is_fav else "收藏"}">{star}</button>'
    )
...
async def paper_detail(request: Request, paper_id: int):
    ...
    # 服务端仅做原始字符串准备，客户端用 marked.js + KaTeX 渲染
    summary_html = ""
    if summary_content:
        # 仍保留以备老模板 fallback，但主渲染走客户端
        summary_html = summary_content
    ...
    return templates.TemplateResponse(template_name, {
        ...
        "is_favorited": db.is_favorited(paper_id),
    })
